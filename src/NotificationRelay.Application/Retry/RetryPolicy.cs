        ...
        for (int attempt = 1; attempt <= _maxRetries; attempt++)
        {
            ...
            catch (Exception ex)
            {
                if (attempt < _maxRetries)
                {
                    ...
                    _logger.LogWarning(
                        "Attempt {Attempt}/{MaxRetries} failed for {Operation} (CorrelationId={CorrelationId}): {Message}. Retrying in {DelayMs}ms.",
                ...
                {
                    _logger.LogError(
                        "All {MaxRetries} attempts exhausted for {Operation} (CorrelationId={CorrelationId}): {Message}",
