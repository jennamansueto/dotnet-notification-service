...
public class NotificationDispatcherTests
{
    ...
    public void SetUp()
    {
        ...
        _emailSender = new InMemoryEmailSender(loggerFactory.CreateLogger<InMemoryEmailSender>());
        _smsSender = new InMemorySmsSender(loggerFactory.CreateLogger<InMemorySmsSender>());
        _teamsSender = new InMemoryTeamsSender(loggerFactory.CreateLogger<InMemoryTeamsSender>());
        _dedupeStore = new InMemoryDeduplicationStore();
        var retryPolicy = new RetryPolicy(
            loggerFactory.CreateLogger<RetryPolicy>(), maxRetries: 2, initialBackoffMs: 1);
